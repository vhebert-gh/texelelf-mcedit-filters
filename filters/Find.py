...
def perform(level, box, options):
	...
	if not search:
		...
		if op == "Dump Found Coordinates":
			...
			campos = search.pop(0)
			editor.mainViewport.cameraPosition = (campos[0]+0.5,campos[1]+2,campos[2]-1)
			editor.mainViewport.yaw = 0.0
			editor.mainViewport.pitch = 45.0

			newBox = BoundingBox(campos, (1,1,1))
			editor.selectionTool.setSelection(newBox)

			if not search:
				addendum = "No further matches were found."
			else:
				if op == "Find Next":
					addendum = "Click \"Filter\" to continue to the next result"
				else:
					addendum = "Additional search results were found. Change \"Operation\" to \"Find Next\" and click \"Filter\" to see the next search result."
			raise Exception("\nMatching blocks/tile entities found at: "+str(campos)+".\n\n"+addendum)
